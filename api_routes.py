...
def setup_routes(app: FastAPI, index):
    ...
    async def delete_user(username: str):
        ...
        try:
            ...
            # Get user information (including ID)
            ...
            if index is not None:
                remove_success = remove_from_faiss_index(index, user_id)
                if not remove_success:
                    print(f"Warning: Could not remove user ID {user_id} from FAISS index")
                else:
                    # Save the updated index
                    try:
                        import faiss
                        faiss.write_index(index, "face_index.bin")
                        print("FAISS index has been updated and saved")
                    except Exception as e:
                        print(f"Error saving FAISS index: {e}")
            
            # Delete the user from the database
            delete_success = delete_user_by_username(username, conn)
            ...
            return JSONResponse(
                ...
                content={
                    "code": 200,
                    "message": "success",
                    "detail": f"User '{username}' has been successfully deleted"
                }
            )
        except HTTPException as he:
            # 直接传递HTTP异常
            ...
            return JSONResponse(
                status_code=500,
                content={"code": 500, "message": f"处理请求时出错: {str(e)}", "detail": None}
            )